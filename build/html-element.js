...
function toolTip(element) {
  ...
  var direction = opts.direction || element.getAttribute('data-direction');
  init();
  ...
  function init() {
    ...
    try {
      for (var _iterator = opts.showToolTipOn[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var event = _step.value;
        if (typeof event === 'string') element.addEventListener(event, show);
      }
    } catch (err) {
      _didIteratorError = true;
      _iteratorError = err;
  ...
  function show(e) {
    ...
    function check(direction) {
      switch (direction) {
        case 'left':
          if (calc('left') < 0 || !checkFromMiddle('top') || !checkFromMiddle('bottom')) return false;
          return true;

        case 'right':
          if (calc('right') + toolTipClient.width > innerWidth || !checkFromMiddle('top') || !checkFromMiddle('bottom')) return false;
          return true;

        case 'top':
          if (calc('top') < 0 || !checkFromMiddle('left') || !checkFromMiddle('right')) return false;
          return true;

        case 'bottom':
          if (calc('bottom') + toolTipClient.height > innerHeight || !checkFromMiddle('left') || !checkFromMiddle('right')) return false;
          return true;
      }
    }
