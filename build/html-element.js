...
for (var key in _html__WEBPACK_IMPORTED_MODULE_1__) {
  exprt[key] = _html__WEBPACK_IMPORTED_MODULE_1__[key];
}
...
function select() {
  ...
  (function init() {
    ...
    function onkeypress(e) {
      var val = value();
      var len = allOptions.length - 1;
      var index = allOptions.indexOf(val);
  ...
  function setvalue(value) {
    ...
    var selected = optionsContainer.querySelector('.__selected');
    if (selected) selected.classList.remove('__selected');
    var option = optionsContainer.querySelector("[data-value='".concat(value, "']"));
